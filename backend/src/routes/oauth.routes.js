...
router.get('/google/user/callback',
    passport.authenticate('google-user', {
        session: false,
        failureRedirect: 'http://localhost:4200/user-landing?error=oauth_failed'
    }),
    (req, res) => {
        try {
            // Generate JWT token
            ...
            res.redirect(`http://localhost:4200/home?token=${token}&oauth=success&newUser=${isNewUser}`);
        } catch (error) {
            console.error('OAuth callback error:', error);
            res.redirect('http://localhost:4200/user-landing?error=token_generation_failed');
        }
    }
);
...
router.get('/google/doctor/callback',
    passport.authenticate('google-doctor', {
        session: false,
        failureRedirect: 'http://localhost:4200/doctor-landing?error=oauth_failed'
    }),
    (req, res) => {
        try {
            // Generate JWT token
            ...
            res.redirect('http://localhost:4200/doctor-landing?error=token_generation_failed');
        }
    }
);
